 ...
 *
 * Implementers should consider subclassing @{link MysqlTaskOutputCommitter} instead of this class.
 */
public abstract class AbstractMysqlOutputCommitter extends OutputCommitter {
  protected final Log log = LogFactory.getLog(getClass());

  private final MysqlOutputFormat verticaOutputFormat;

  /**
   * Initializes the OutputCommitter with a handle to the MysqlOutputFormat so the connection can
   ...
   *
   * @param verticaOutputFormat the MysqlOutputFormat object
   */
  public AbstractMysqlOutputCommitter(MysqlOutputFormat verticaOutputFormat) {
    if (verticaOutputFormat == null) {
      throw new IllegalArgumentException("MysqlOutputFormat must not be null");
    }
    this.verticaOutputFormat = verticaOutputFormat;
  }

  /**
   * Fetches and returns the connection from the MysqlOutputFormat. The connection will be lazily
   * opened and cached.
   ...
   * {@link JobStatus.State#SUCCEEDED}. If this method throws an
   ...
   * {@link JobStatus.State#FAILED} or
   * {@link JobStatus.State#KILLED} and might be called more than once.
   */
  @Override
