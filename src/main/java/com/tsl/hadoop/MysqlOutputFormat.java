...
public class MysqlOutputFormat extends OutputFormat<Text, MysqlRecord> {
  ...
  /**
	...
	public static void setOutput(Job job, String tableName, boolean truncateTable) {
		setOutput(job, tableName, truncateTable, null);
	}
	...
	/**
	  ...
	  */
	public static void setOutput(Job job, String tableName, 
			boolean truncateTable, String sql) {
		MysqlConfiguration vtconfig = new MysqlConfiguration(
				job.getConfiguration());
		vtconfig.setOutputTableName(tableName);
		//vtconfig.setOutputTableDef(tableDef);
        vtconfig.setOutputTableSql(sql);
	...
	public void checkOutputSpecs(JobContext context) throws IOException {
		checkOutputSpecs(new MysqlConfiguration(context.getConfiguration()));
	}
...
//			}
  	...
  	/** {@inheritDoc} */
	public RecordWriter<Text, MysqlRecord> getRecordWriter(
			TaskAttemptContext context) throws IOException {

		MysqlConfiguration config = new MysqlConfiguration(
				context.getConfiguration());
        String sql = config.getOutputTableSql();
		try {
			return new MysqlRecordWriter(getConnection(context.getConfiguration()), sql, config.getBatchSize());
		} catch (SQLException e) {
	...
	public static void optimize(Configuration conf) throws Exception {
		MysqlConfiguration vtconfig = new MysqlConfiguration(conf);
		Connection conn = vtconfig.getConnection(true);
    ...
    MysqlConfiguration config = new MysqlConfiguration(context.getConfiguration());
    ...
    try {
      ...
      throw new IOException("Could not find OutputCommitter class confiugured as " +
        MysqlConfiguration.OUTPUT_COMMITTER_CLASS_PARAM, e);
    }
	}

  /**
   * Given a class, initializes it and returns it. If the class is a subclass of
   * @{link AbstractMysqlOutputCommitter}, then the constructor that takes a
   * @{link MysqlOutputFormat} object will be used. Alternatively, the class can implement
  ...
  private OutputCommitter initializeOutputCommitter(Class outputCommitterClass,
    ...
    // If class extends AbstractMysqlOutputCommitter, create one of those
    // else it needs to be an instance of OutputCommitter
    // else throw exception
    if (AbstractMysqlOutputCommitter.class.isAssignableFrom(outputCommitterClass)) {
      ...
      for (Constructor constructor : outputCommitterClass.getDeclaredConstructors()) {
        if (constructor.getGenericParameterTypes().length == 1 &&
          constructor.getGenericParameterTypes()[0].equals(MysqlOutputFormat.class)) {
          try {
      ...
      throw new IOException("Implementation of AbstractMysqlOutputCommitter must " +
                            "have a constructor that takes a MysqlOutputFormat object");
  ...
  synchronized Connection getConnection(Configuration configuration) throws IOException {
    ...
    MysqlConfiguration config = new MysqlConfiguration(configuration);
    try {
