...
public class MysqlInputSplit extends InputSplit implements Writable {
  private static final Log LOG = LogFactory.getLog("com.tsl.hadoop");

  PreparedStatement stmt = null;
  Connection connection = null;
  MysqlConfiguration vtconfig = null;
  ...
  public MysqlInputSplit() {
	...
	/**
	  ...
	  */
	public MysqlInputSplit(String inputQuery, List<Object> segmentParams) {
	...
	public MysqlInputSplit(String inputQuery, long start, long end) {
  ...
  public void configure(Configuration conf) throws Exception {
    ...
    vtconfig = new MysqlConfiguration(conf);
  ...
  public Configuration getConfiguration() {
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (paramCount > 0) {
    		...
    		for (int i = 0; i < paramCount; i++) {
				segmentParams.add(MysqlRecord.readField(type, in));
			}
		}
		start = in.readLong();
		end = in.readLong();
	...
	public void write(DataOutput out) throws IOException {
		...
		if (segmentParams != null && segmentParams.size() > 0) {
			...
			int type = MysqlRecord.getType(segmentParams.get(0));
			out.writeInt(type);
			for (Object o : segmentParams)
				MysqlRecord.write(o, type, out);
		} else {
