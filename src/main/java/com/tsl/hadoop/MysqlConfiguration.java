...
public class MysqlConfiguration {
	...
	public static final String VERTICA_DRIVER_CLASS = "com.mysql.jdbc.Driver";
	...
	public static final String OUTPUT_TABLE_SQL = "mapred.vertica.output.table.sql";
	...
	public static final String DEFAULT_OUTPUT_COMMITTER = MysqlTaskOutputCommitter.class.getCanonicalName();
	...
	public MysqlConfiguration(Configuration conf) {
		this.conf = conf;
	}
	...
	public Connection getConnection(boolean output) throws IOException,
      ClassNotFoundException, SQLException {
		...
		Connection connection = DriverManager.getConnection("jdbc:mysql://"
				+ hosts[r.nextInt(hosts.length)] + ":" + port + "/" + database + "?loglevel=" + level+"&zeroDateTimeBehavior=convertToNull",
				 user, pass);
		...
		if (query_params != null) {
			...
			for (String str_params : query_params) {
				...
				for (int count = 0; count < sz; count++) {
					int type = in.readInt();
					params.add(MysqlRecord.readField(type, in));
				}
				values.add(params);
			}
		}
	...
	*/
		...
		throws IOException 
	{
		...
		for (List<Object> params : segment_params) {
			...
			{
				int type = MysqlRecord.getType(obj);
				out.writeInt(type);
				MysqlRecord.write(obj, type, out);
			}
	...
	public String getOutputTableSql(){
	    return conf.get(OUTPUT_TABLE_SQL);
    }

    public void setOutputTableSql(String sql){
        if(sql == null || sql.length() == 0) return;
        conf.set(OUTPUT_TABLE_SQL,sql);
    }
